function percentage_correct = neuralnet_genetic(train_data,test_data,structure) 
...
%Creating random weight values
    for h=2:no_layers
        weights{h} = unifrnd(-0.5,0.5,structure(h),structure(h-1)); 
    end
...
iterations = 30000;
    ...
    for m=1:iterations
           ...
           pre_final_layer = beforelastlayer;
            ...
            learning_rate = 0.0002   -  m *  learning_decay * 0.0003; 
            ...
            for l=length(structure):(-1):2
                ...
                weights{l} = weights{l} - learning_rate * weight_changeavg' - 0.001 * weights{l};
